...
def main():
    ...
    if args.quantize:
        ...
        if args.vision_flatquant:
            ...
            logger.info("Finished applying FlatQuant to vision encoder.")
        if args.resume:
            ...
            if args.vision_flatquant:
                processor = transformers.AutoProcessor.from_pretrained(
                    args.model, trust_remote_code=True, **({"token": args.hf_token} if args.hf_token else {})
                )
                vision_samples = data_utils.get_vision_calib_loader(
                    args.cali_dataset_vision, processor, nsamples=args.nsamples_vision
                )
                logger.info(f"Loaded {len(vision_samples)} vision calibration images.")
                cali_flat_quant_vision(args, model, vision_samples, utils.DEV, logger=logger)
